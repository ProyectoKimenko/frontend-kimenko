import {
    ...
    FetchReportParams,
    ReportResponse,
} from "../types/helpers/typesFetchAnalysis";

const API_BASE_URL = process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000";

export const fetchAnalysis = async (
    params: FetchAnalysisParams
...
export const fetchReport = async (
    ...
    try {
        const response = await fetch(
            `${API_BASE_URL}/generate_weekly_pdf?${urlParams.toString()}`,
            {
                method: "GET",
                headers: {
...
export type StackplotRow = {
    time_bucket: string;
    [key: string]: string | number;
};

export type StackplotResponse = {
    place_id?: number;
    granularity?: string;
    categories: string[];
    data: StackplotRow[];
...
export const fetchStackplotData = async (
    ...
    const urlParams = new URLSearchParams();
    urlParams.append("place_id", String(placeId));
    urlParams.append("start_date", startDate);
    urlParams.append("end_date", endDate);
    urlParams.append("granularity", granularity);

    try {
        const response = await fetch(
            `${API_BASE_URL}/analysis/stackplot?${urlParams.toString()}`,
            {
                method: "GET",
                headers: {
                    Accept: "application/json",
        ...
        if (!response.ok) {
            ...
            throw new Error(
                `Error fetching stackplot data: ${response.status} ${response.statusText} - ${errorText}`
            );
        }

        return (await response.json()) as StackplotResponse;
    } catch (error) {
        console.error("Failed to fetch stackplot data:", error);
        throw error;
    }
};